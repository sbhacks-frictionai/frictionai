...
- Output study question only at the end of the study guide.
...
async function generateStudyGuideWithGemini(
	...
	try {
		const response = await fetch(geminiUrl, {
			...
			body: JSON.stringify({
				...
				generationConfig: {
					...
					topK: 40,
					topP: 0.95,
					maxOutputTokens: 8192, // Longer output for comprehensive study guides
				},
				safetySettings: [
					{
		...
		const studyGuide = candidate?.content?.parts?.[0]?.text;
		...
		if (!studyGuide) {
			console.error(
				"No study guide generated. Finish reason:",
				finishReason
			);
...
async function generateStudyGuideWithContinuation(
	...
	for (let i = 0; i < MAX_CONTINUATIONS; i++) {
		console.log(
			`Generating study guide chunk ${i + 1}/${MAX_CONTINUATIONS}...`
		);
		const chunk = await generateStudyGuideWithGemini(prompt, apiKey);

		if (!chunk || !chunk.trim()) {
			console.log(`Chunk ${i + 1} is empty, stopping generation`);
			break;
		}
		...
		if (!chunk.trim().endsWith("CONTINUE")) {
			console.log(
				`Study guide generation completed after ${i + 1} chunk(s)`
			);
			return fullText;
		}

		console.log(
			`Chunk ${i + 1} ended with CONTINUE, generating next chunk...`
		);
